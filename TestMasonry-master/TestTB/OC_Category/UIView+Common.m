...
- (void)configBlankPage:(EBlankPageType)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void (^)(id))block {
    if (hasData) {
...
- (void)configWithType:(EBlankPageType)blankPageType hasData:(BOOL)hasData hasError:(BOOL)hasError reloadButtonBlock:(void (^)(id))block {
    ...
    if (hasError) {
        ...
        _tipLabel.text = @"加载失败，您可以点击刷新重新加载";
    } else {
        //空白数据
        if (_reloadButton) {
        ...
        switch (blankPageType) {
            case EBlankPageType_NoMsg: {
                imageName = @"blankpage_no_message";
                tipStr = @"没有";
            }
                break;
            case EBlankPageType_NoDish: {
                imageName = @"blankpage_no_dish";
                tipStr = @"没有\n";
            }
                break;
            case EBlankPageType_None: {
                imageName = @"";
                tipStr = @"";
            }
                break;
